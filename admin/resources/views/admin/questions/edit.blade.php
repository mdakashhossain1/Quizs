...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Edit Question</h1>
            <p class="text-xs text-slate-400 mt-0.5">Modify question text, points, and answer choices</p>
        </div>
        <a href="{{ route('admin.questions.index', ['quiz_id' => $question->quiz_id]) }}" class="text-xs font-semibold text-slate-500 hover:text-slate-800">&larr; Back to list</a>
    </div>
        ...
        <form action="{{ route('admin.questions.update', $question) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="question_text" name="question_text" rows="3" required
                          class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('question_text', $question->question_text) }}</textarea>
            </div>

            <div>
                <label for="explanation" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Explanation</label>
                <textarea id="explanation" name="explanation" rows="2"
                          class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('explanation', $question->explanation) }}</textarea>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-400">Review choices and verify the correct radio button selection</p>
                    </div>
                </div>

                <div class="space-y-3">
                    @php
                        $optionsList = $question->options->values();
                    @endphp
                    @for($i = 0; $i < 4; $i++)
                        @php
                        ...
                        <div class="flex items-center gap-3 p-3.5 bg-slate-50/80 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
                            <label class="flex items-center gap-2 cursor-pointer pr-2 border-r border-slate-200">
                                <input type="radio" name="correct_option" value="{{ $i }}" {{ old('correct_option', $isSelected ? $i : 0) == $i ? 'checked' : '' }} required
                            ...
                            <input type="text" name="options[{{ $i }}]" value="{{ old("options.{$i}", $currentOption->option_text ?? '') }}" required placeholder="Option {{ $i + 1 }} text..."
            ...
            <div>
                ...
                <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $question->sort_order) }}" required
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                <a href="{{ route('admin.questions.index', ['quiz_id' => $question->quiz_id]) }}" class="px-5 py-2.5 text-xs font-semibold text-slate-600 hover:text-slate-900">Cancel</a>
